        ...
        with tqdm(train_dataloader, unit='batch') as loader:
            ...
            for ind, (face, gaze, head_pose, tag) in enumerate(loader):
                ...
                with autocast(device_type='cuda', enabled=True):
                    ...
                    if self.config.train_setting.using_detr_gz_loss:
...
def running(config):
    ...
    train_loader = get_dataloader(config.data, config.data.trainset_setting)
    ...
    for epoch in range(1, config.train_setting.epoch+1):
        loss = GazeModel.train(train_loader, epoch)
        error = GazeModel.test(test_loader, prev_p=prev_p, prev_z=prev_z)
...
def trained_model_cross_domain(config):
    ...
    model_state_dict = f'./model/resnet18.pth'
    error = GazeModel.test(test_loader, model_state_dict=model_state_dict)
    print(error)
